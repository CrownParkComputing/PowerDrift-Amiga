...
/* Black Tiger memory map for z80.c's inline fast path (shared by BOTH CPUs).
 *   Main:  0x0000-0xbfff fixed+banked ROM, 0xc000-0xcfff bg videoram (a BANKED
 *          0x1000 window into a 0x4000 scrollram -- must trap), 0xd000-0xd7ff
 *          txram, 0xd800-0xdfff RAM palette, 0xe000-0xfdff RAM, 0xfe00-0xffff
 *          spriteram. Control/scroll/bank are PORT-mapped (IN/OUT), not memory.
 *   Audio: 0x0000-0x7fff ROM, 0xc000-0xc7ff RAM, 0xc800 soundlatch (read),
 *          YM2203 regs at 0xe000-0xe003.
 * Reads: trap the 0xc000 page (main bg-ram banked window + audio latch at
 * 0xc800); all else plain memory[]. Writes: plain for txram/palette/RAM/
 * spriteram (0xd000-0xffff) and audio RAM (0xc000-0xc7ff handled by the trap is
 * fine too); trap the 0xc000-0xcfff bg window, ROM, AND the audio YM 0xe000-3.
 * NOTE: the audio CPU's YM regs at 0xe000-0xe003 sit in the main CPU's RAM
 * region. We trap that 4-byte window on BOTH reads and writes so the audio core's
 * YM accesses reach machine_rd/wr (cblktiger_audio.c dispatches by context, like
 * 1943): writes go to the YM chips (else silently fast-written to memory[] -> NO
 * sound), and READS return the YM status register (the sound driver's RST38 timer
 * IRQ handler polls 0xe000 BIT 0/1 for the timer-A/B overflow flag -- without the
 * read trap it sees 0 forever and never plays a note). Trapping those 4 addresses
 * for the MAIN CPU is harmless (machine_rd/wr just hit memory[a] for its work RAM).
 * All other 0xd000+ writes / non-0xc000-page reads stay on the fast path. */
#define Z80_FAST_READ(a)  ((((a) & 0xf000u) != 0xc000u) && !((a) >= 0xe000u && (a) <= 0xe003u))
#define Z80_FAST_WRITE(a) (((a) >= 0xd000u) && !((a) >= 0xe000u && (a) <= 0xe003u))
#endif
#if defined(Z80_MAP_SIDEARMS) && !defined(Z80_FAST_READ)
/* Side Arms memory map for z80.c's inline fast path (shared by BOTH CPUs).
 *   Main:  0x0000-0xbfff fixed+banked ROM, 0xc000-0xc7ff palette RAM (xBRG_444,
 *          CPU-written -- renderer reads it straight from memory[]), 0xc800-0xc80c
 *          I/O (inputs/latch/bank/control/scroll/gfxctrl), 0xd000-0xdfff fg
 *          videoram+colorram, 0xe000-0xefff RAM, 0xf000-0xffff spriteram.
 *   Audio: 0x0000-0x7fff ROM, 0xc000-0xc7ff RAM, 0xd000 soundlatch (read),
 *          YM2203 regs at 0xf000-0xf003.
 * Reads: trap the 0xc000 page so the input/DSW ports at 0xc800-0xc805 reach
 * machine_rd (palette c000-c7ff reads just return memory[a]), plus audio latch
 * d000 and YM status f000-f003 for the sound CPU.
 * Writes: plain for palette (c000-c7ff) and vram/cram/ram/spr (0xd000-0xffff);
 * trap ROM (<0xc000, ignored) and the 0xc800-0xcfff I/O block. When the audio
 * core is added its YM regs at 0xf000-3 sit in the main CPU's spriteram region,
 * so build the audio side to trap those (dispatch by context, like 1943). */
#define Z80_FAST_READ(a)  ((((a) & 0xf000u) != 0xc000u) && ((a) != 0xd000u) && !((a) >= 0xf000u && (a) <= 0xf003u))
#define Z80_FAST_WRITE(a) ((((a) >= 0xc000u && (a) <= 0xc7ffu) || (a) >= 0xd000u) && !((a) >= 0xf000u && (a) <= 0xf003u))
#endif
